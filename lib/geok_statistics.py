# -*- coding: utf-8 -*-
"""
Created on Sat Apr  6 13:47:26 2019
...
import conversion_time as cnv_tim
...
def time_win_basic(start,end,Tlisin,Datalisin,outposix=True,invert=False,
    ...
    if isinstance(Tlisin[0],dt.datetime):
        Tlis  = cnv_tim.dt2posix(Tlisin)
    else:
        Tlis = Tlisin


    if isinstance(start,dt.datetime):
        start = cnv_tim.dt2posix(start)
    if isinstance(end,dt.datetime):
        end   = cnv_tim.dt2posix(end)
    ...
    if only_boolis:
        ...
        if not outposix:
            Tlisout = cnv_tim.posix2dt(Tlisout)
